...
subroutine USER_wind_forcing(sfc_state, forces, day, G, US, CS)
  ...
  integer :: isd, ied, jsd, jed, IsdB, IedB, JsdB, JedB ! by me !
  integer :: isg, ieg, jsg, jeg, IsgB, IegB, JsgB, JegB ! by me !
  integer :: j_50, Jq_50 
  real :: yfu, yfv
  real :: m, tau0, PI, len_lat, asym
  real :: x, y 
  ...
  !  
  j_50 = int((jeg - jsg) / 2) + jsg
  Jq_50 = int((JegB - JsgB) / 2) + JsgB
  ! 
  len_lat = G%len_lat !G%geoLatCu(is,je) - G%GeoLatCu(is,js) ! 2. * yf
  yfu = G%gridLatT(j_50) ! len_lat/2.0
  yfv = G%gridLatB(Jq_50)! len_lat/2.0
  ! write(*,*) G%geoLatCu(is,je), len_lat, G%len_lat, G%south_lat
  ! write(*,*) jsg,jeg,j_50, JsgB,JegB,Jq_50, yfu, yfv 
  ...
  tau0 = 0.22 *US%kg_m3_to_R*US%m_s_to_L_T**2*US%L_to_Z  ! was 0.08 in [R L Z T-2 ~> Pa]
  m = 0.1    ! tilt 
  asym = 0.0 ! Asymmetry of the wind
  
  ! set the surf wind stress
  do j=js,je ; do I=is-1,Ieq   !is-1,Ieq
    x = G%geoLonCu(I,j) - G%west_lon
    y = G%geolatCu(I,j) - G%south_lat
    forces%taux(I,j) = G%mask2dCu(I,j) * 0.5* tau0 * & 
                       ( 1.0 + cos(2.0*PI*(m*x-y+yfu) / ((1.+m)*len_lat)) + &
                       asym*y/len_lat ) 
   ! forces%taux(I,j) = G%mask2dCu(I,j) * tau0 * &
   !                   ( cos(2.0*PI*(m*x-y+yfu) / ((1.+m)*len_lat)) + &
   !                    asym*y/len_lat )

  enddo ; enddo
  do J=js-1,Jeq ; do i=is,ie !js-1,Jeq
    x = G%geoLonCv(i,J) - G%west_lon
    y = G%geoLatCv(i,J) - G%south_lat
   ! forces%tauy(i,J) = 0.0
    forces%tauy(i,J) = G%mask2dCv(i,J) * 0.5 * tau0 * m * &
                      ( 1.0 + cos(2.0*PI*(m*x-y+yfv) / ((1.+m)*len_lat)) + &
                       asym*y/len_lat )
   ! forces%tauy(i,J) = G%mask2dCv(i,J) * tau0 * m * &
   !                   ( cos(2.0*PI*(m*x-y+yfv) / ((1.+m)*len_lat)) + &
    !                   asym*y/len_lat )
  enddo ; enddo
