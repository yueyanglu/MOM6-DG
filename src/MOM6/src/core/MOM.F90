...
subroutine step_MOM_tracer_dyn(CS, G, GV, US, h, Time_local)
  ...
  ! added by YLU
  ! print*, 't_dyn_rel_adv = ', CS%t_dyn_rel_adv (DT_THERMAL)
  ! add end
  ...
  ! add relaxation frocing by YLU 
  call tr_relax_user(h, CS%t_dyn_rel_adv, G, GV, CS%tracer_Reg)
  ! add end
...
subroutine step_offline(forces, fluxes, sfc_state, Time_start, time_interval, CS)
  ...
  if (CS%use_ALE_algorithm) then
    ...
    call update_offline_fields(CS%offline_CSp, CS%h, fluxes, CS%use_ALE_algorithm)
    !call MOM_mesg("After read in new effline fields==============")
...
!--- relaxation forcing to the upper-layer tracer (for online), by yueyang
subroutine tr_relax_user(h, dt, G, GV, Reg)
! d(ch)/dt + ... = h*r*(c_ref - c)
! c2 = ( c1*h1 + dt * h2*r*(c_prof- c) ) / h2
  type(ocean_grid_type),                     intent(in) :: G   !< The ocean's grid structure
  type(verticalGrid_type),    intent(in)    :: GV      !< ocean vertical grid structure
  real, dimension(SZI_(G),SZJ_(G),SZK_(GV)), & !id,jd,nz
                           intent(in)    :: h !< Layer thickness (see 'tracer_hordiff')
  ...
  integer :: isd, ied, jsd, jed
  real :: x, y, rate, &
          x0, y0  ! westmost and southmost position of P-cell [m]
  real :: h_neglect, PI, c0, iw_y, Ly
  real, dimension(SZI_(G),SZJ_(G))           :: tr_ref, dTr    
  ...
  c0 = 20. 
  iw_y = 3./16.
  ! note that G%geoLatT is segmentated!!!  in [km]
  Ly = G%gridLatT(g%jeg) - G%gridLatT(g%jsg) 
  x0 = G%gridLonT(g%isg) 
  y0 = G%gridLatT(g%jsg)
  ! print*,'g%jsg=', g%jsg, 'g%jeg=', g%jeg
  ! print*, 'Ly=', Ly, 'x0=', x0, 'y0=', y0
  ! print*, 'gridLatT: ', G%gridLatT(g%jsg:g%jeg)

  ! ------------ create idealized relax profile ------------
  tr_ref(:,:) = 0.0
  do j=js,je ; do i=is,ie
    x = G%geoLonT(i,j) - x0
    y = G%geoLatT(i,j) - y0 
    tr_ref(i,j) = c0 * COS(2.0*PI*iw_y * y/Ly)
  enddo ; enddo
  !print*,'is, js, je=',is, js, je
  !print*, 'tr_ref(is,js:je)=', tr_ref(is, js:je)

  ! ------------ Apply the relax forcing ------------
  !Reg%Tr(1)%t(22,22,1) = 0.1
  !print*, 'Reg%Tr(m)%t(22,22,1)',Reg%Tr(1)%t(22,22,1)
  dTr(:,:) = 0.0
  do m = ntr,ntr
    !print*, 'ntr', ntr
    do j=js,je ; do i=is,ie
      dTr(i,j) = rate*dt * (tr_ref(i,j) - Reg%Tr(m)%t(i,j,1))
      Reg%Tr(m)%t(i,j,1) =  Reg%Tr(m)%t(i,j,1) + dTr(i,j) 
    enddo ; enddo
  enddo

end subroutine tr_relax_user
