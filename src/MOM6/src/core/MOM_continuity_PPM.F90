...
!!!---- These two subroutines are added by the USER ----!!! 
...
subroutine h_relax_user_tilt(h,LB,G,ni,nj)
  ...
  !call get_param(param_file, "MOM", "relax_const", relax_const, &
  !               "Rate of relaxation", default=5e-5)
  !print*, 'relax_const=', relax_const
  !call get_param(param_file, mod, "delta_h", delta_h, &
  !               "1/2 size of SSH sine curve", default=80.0)
  relax_const = 1.0e-6 !1.0e-6 for DT=50s  
  ...
  ! print *, L_pro, yf, G%geoLatT(is,j_50), G%geoLatCu(is,j_shift+1) - G%geoLatCu(is,1) ! has been tested
  !==
  !print *, relaxation of h1 applied...
  !relax_const = 1.0e-6
  !dh = 120.0
  !m = 0.05
  ...
  do j=js,je ; do i=is,ie
    ...
    if (m * x + yf - 0.6 * L_pro < y - y_shift .AND. y - y_shift < m * x + yf + 0.6 * L_pro) then
      ! apply the forcing to h1 across the whole horizontal domain
...
!--- imposes mass conservation to the upper layer
...
subroutine continuity_PPM(u, v, hin, h, uh, vh, dt, G, GV, US, CS, uhbt, vhbt, OBC, &
  ...
  !!! ------ added by USER ------ !!!
  ! relaxation of upper layer thickness and conservation !
  nj = LB%jeh - LB%jsh + 1
  ni = LB%ieh - LB%ish + 1
